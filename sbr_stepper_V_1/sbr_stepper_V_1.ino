// This program balances the self balancing wheeled robot using stepper motor using two cascading loop of PID controller
// outer loop is velocity control and inner loop is tilt control
...
float setpoint_th=-4.35, error_th, p_error_th, I_th, PID_th, kp_th, ki_th, kd_th;
float setpoint_w=0, error_w, p_error_w, I_w, PID_w, kp_w, ki_w, kd_w;

float SampleFrequency = 330, dt = 1/SampleFrequency, loop_timer = 1000000*dt;
unsigned long t;
unsigned long start;
...
void loop(){
  ...
  error_w = setpoint_w-(-PID_th);
  I_w = I_w + ki_w*error_w; 
  if(I_w>15)I_w=15;
  else if(I_w<-15)I_w=-15;
  PID_w = kp_w*error_w + I_w + kd_w*(error_w-p_error_w);
  if(PID_w>15)PID_w=15;
  else if(PID_w<-15)PID_w=-15;
  //Serial.print(1*(error_w-p_error_w));Serial.print("\t");Serial.println(-PID_th);
  p_error_w = error_w;
  
  setpoint_th = PID_w-4.35;
